...
function App() {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        const response = await fetch('https://country-guessr-12.vercel.app/api/countries');
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const res = await fetch('https://country-guessr-12.vercel.app/api/users/login', {
        method: 'POST',
  ...
  const fetchLeaderboard = async () => {
    try {
      const res = await fetch('https://country-guessr-12.vercel.app/api/users/leaderboard');
  ...
  const saveScoreAndEndGame = async (finalScore) => {
    ...
    try {
      if (currentUser && finalScore > 0) {
        const res = await fetch('https://country-guessr-12.vercel.app/api/users/score', {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
